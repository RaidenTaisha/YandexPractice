...
class Parking {
 ...
 public:
  ...
  void Park(VehiclePlate car) {
    ...
    now_parked_.insert({car, Clock::now()});
  }
  ...
  void Withdraw(const VehiclePlate& car) {
    ...
    complete_parks_.insert({car, Clock::now() - now_parked_.at(car)});
    now_parked_.erase(car);
  }

  // получить счёт за конкретный автомобиль
  int64_t GetCurrentBill(const VehiclePlate& car) const {
    if (complete_parks_.find(car) != complete_parks_.end() && now_parked_.find(car) != now_parked_.end()) {
      return chrono::duration_cast<chrono::seconds>(complete_parks_.at(car)).count() * cost_per_second_
        + chrono::duration_cast<chrono::seconds>(Clock::now() - now_parked_.at(car)).count() * cost_per_second_;
    } else if (complete_parks_.find(car) != complete_parks_.end()){
      return chrono::duration_cast<chrono::seconds>(complete_parks_.at(car)).count() * cost_per_second_;
    } else if (now_parked_.find(car) != now_parked_.end()) {
      return chrono::duration_cast<chrono::seconds>(Clock::now() - now_parked_.at(car)).count() * cost_per_second_;
    }
    return 0;
  }
  ...
  unordered_map<VehiclePlate, int64_t, VehiclePlateHasher> EndPeriodAndGetBills() {
    unordered_map<VehiclePlate, int64_t, VehiclePlateHasher> car_to_bill;
    for (auto &[car, time_point] : now_parked_) {
      complete_parks_[car] += Clock::now() - time_point;
      time_point = Clock::now();
    }
    for (auto &[car, _] : complete_parks_) {
      car_to_bill.insert({car, GetCurrentBill(car)});
    }
