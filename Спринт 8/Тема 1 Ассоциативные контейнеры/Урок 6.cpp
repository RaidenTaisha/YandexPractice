...
struct PlateHasherAll {
 public:
  size_t operator()(const VehiclePlate& plate) const {
    size_t hash = 1;
    for (auto &letter : plate.GetLetters()) {
      hash *= static_cast<size_t>(letter);
    }
    return hash * static_cast<size_t>(plate.GetDigits() + plate.GetRegion() * 1000);
  }
};
...
class PlateGenerator {
  char GenerateChar() {
    ...
    return possible_chars_[static_cast<size_t>(char_gen(engine_))];
  }
  ...
  int GenerateRegion() {
    ...
    return possible_regions_[static_cast<size_t>(region_gen(engine_))];
  }
...
int main() {
  ...
  int found;
  {
    ...
    found = static_cast<int>(count_if(find_vector.begin(), find_vector.end(), [&](const VehiclePlate& plate) {
