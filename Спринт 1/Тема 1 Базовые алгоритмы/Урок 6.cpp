...
vector<pair<int, int>> FindTopDocuments(const vector<pair<int, vector<string>>>& documents,
  ...
  vector<pair<int,int>> founded_documents = FindAllDocuments(documents, stop_words, raw_query);
  for (int i = 0; i < founded_documents.size(); i++) {
    founded_documents[i] = { founded_documents[i].second, founded_documents[i].first };
  }
  sort(founded_documents.begin(), founded_documents.end());
  reverse(founded_documents.begin(), founded_documents.end());
  vector<pair<int,int>> ret;
  for (int i = 0; i < founded_documents.size() && i < MAX_RESULT_DOCUMENT_COUNT; i++) {
    ret.push_back({ founded_documents[i].second, founded_documents[i].first });
  }
  return ret;
}
