...
// A * (B / C) - always okay (the parent has the highest grammatic precedence)
// A / (B * C) - never okay A / (B / C) - never okay
// -(A + B) - never okay
// -(A - B) - never okay
...
public:
  ...
  virtual double Evaluate(CallbackFunction callback) const = 0;
...
public:
  ...
  void
  DoPrintFormula(std::ostream &out, ExprPrecedence precedence) const override {
...
// При делении на 0 выбрасывайте ошибку вычисления FormulaError
  double Evaluate(CallbackFunction callback) const override {
    using std::string_literals::operator""s;

    auto rhs_evaluate = rhs_->Evaluate(callback);
    auto lhs_evaluate = lhs_->Evaluate(callback);
    double res;

    switch (type_) {
      case Add:
        res = lhs_evaluate + rhs_evaluate;
        break;
...
double FormulaAST::Execute(FormulaAST::CallbackFunction callback) const {
  return root_expr_->Evaluate(callback);
}
