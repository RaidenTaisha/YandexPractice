...
void TestFormulaReferencedCells() {
  ...
  ASSERT_EQUAL(tricky->GetReferencedCells(), (std::vector{"A1"_pos, "A2"_pos,
    "A3"_pos}));
}
...
void CreateAndClearCells() {
  auto sheet = CreateSheet();
  sheet->SetCell("A1"_pos, "1");
  sheet->SetCell("B2"_pos, "=2+2");
  sheet->SetCell("C3"_pos, "meow");
  sheet->SetCell("A3"_pos, "=A1+B2");
  ...
  {
    auto a1 = sheet->GetCell("A1"_pos);
    ASSERT_EQUAL(a1->GetText(), "1")
    ASSERT_EQUAL(std::get<std::string>(a1->GetValue()), "1")
  }

  {
    auto b2 = sheet->GetCell("B2"_pos);
    ASSERT_EQUAL(b2->GetText(), "=2+2")
    ASSERT_EQUAL(std::get<double>(b2->GetValue()), 4)
  }

  {
    sheet->SetCell("A3"_pos, "=A1+B2");
    auto a3 = sheet->GetCell("A3"_pos);
    ASSERT_EQUAL(a3->GetText(), "=A1+B2")
    ASSERT_EQUAL(std::get<double>(a3->GetValue()), 5)
  }
}

void CheckValuesAfterCreateAndClearCells() {
  auto sheet = CreateSheet();
  sheet->SetCell("A1"_pos, "1");
...
int main() {
  ...
  RUN_TEST(tr, CreateAndClearCells);
  RUN_TEST(tr, CheckValuesAfterCreateAndClearCells);
