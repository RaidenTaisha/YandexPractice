...
class Octopus {
 public:
  ...
  Octopus(const Octopus& other) {
    ...
    tentacles_.reserve(other.tentacles_.size());

    try {
      for (auto t : other.tentacles_) {
        // Копируем объект, если указатель на него ненулевой
        auto t_copy = t ? new Tentacle(*t) : nullptr;  // new может выбросить
        // исключение

        // Не выбросит исключение, т. к. в vector память уже зарезервирована
        tentacles_.push_back(t_copy);
      }
    } catch (...) {
  ...
  Tentacle& AddTentacle() {
    ...
    auto* t = new Tentacle(static_cast<int>(tentacles_.size())+1);
