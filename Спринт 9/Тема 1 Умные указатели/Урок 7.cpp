...
class Cache {
 public:
  ...
  explicit Cache(ValueFactoryFn value_factory) : fabric_(value_factory) {}
  ...
  std::shared_ptr<Value> GetValue(const Key& key) {
    ...
    if (cache_.count(key) == 0 || cache_.at(key).expired()) {
      value_ptr = fabric_(key);
      cache_[key] = value_ptr;
    }
...
class BookLoader {
 public:
  ...
  explicit BookLoader(const BookStore& store, size_t& load_count)
      : store_(store), counter_(load_count) {}
