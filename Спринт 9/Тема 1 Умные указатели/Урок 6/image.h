...
class Image {
 public:
  ...
  explicit Image(Size size, char color = ' ')
      : size_{size.width > 0 && size.height > 0 ? size : throw std::out_of_range("Invalid image size")},
        size_in_tiles_{(size_.width + Tile::SIZE - 1) / Tile::SIZE, (size_.height + Tile::SIZE - 1) / Tile::SIZE},
        tiles_{static_cast<size_t>(size_in_tiles_.width * size_in_tiles_.height), CoWTile{Tile{color}}} {
  }
  ...
  char GetPixel(Point p) const noexcept {
    if (p.x >= size_.width || p.y >= size_.height || p.x < 0 || p.y < 0)
      return ' ';
    Point tile {(p.x / Tile::SIZE), (p.y / Tile::SIZE)};
    return tiles_.at(GetTileIndex(p))->GetPixel({p.x - tile.x * Tile::SIZE, p.y - tile.y * Tile::SIZE});
  }
  ...
  void SetPixel(Point p, char color) {
    if (p.x < size_.width && p.y < size_.height && p.x >= 0 && p.y >= 0) {
      Point tile {(p.x / Tile::SIZE), (p.y / Tile::SIZE)};
      tile.x = p.x - tile.x * Tile::SIZE;
      tile.y = p.y - tile.y * Tile::SIZE;
      tiles_.at(GetTileIndex(p)).Write()->SetPixel(tile,color);
    }
  }
