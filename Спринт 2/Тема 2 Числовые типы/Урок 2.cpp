...
class SearchServer {
 public:
  ...
  void AddDocument(int document_id,
    ...
    id_to_average_rating_[document_id] = ComputeAverageRating(ratings);
 ...
 private:
  ...
  map<int, int> id_to_average_rating_;
  ...
  static int ComputeAverageRating(const vector<int> &ratings) {
    int ret = 0;
    if (!ratings.empty()) {
      ret = accumulate(ratings.begin(), ratings.end(), 0)
  ...
  vector<Document> FindAllDocuments(const Query &query) const {
    ...
    for (const auto &[document_id, relevance] : document_to_relevance) {
      matched_documents.push_back({document_id,
                                      id_to_average_rating_.at(document_id),
                                      relevance});
    }
    return matched_documents;
  }
...
vector<int> ReadRatings() {
  int num_of_ratings = ReadNumber();
  vector<int> ratings;
  if (num_of_ratings != 0) {
    for (int i = 0; i < num_of_ratings; i++) {
      ratings.push_back(ReadNumber());
    }
  }
  ReadLine();
  return ratings;
...
int main() {
  ...
  for (auto [document_id, rating, relevance]
