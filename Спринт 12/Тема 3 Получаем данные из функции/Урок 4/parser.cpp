...
class PayTax : public ModifyQuery {
 public:
  PayTax(Date from, Date to, double rate) : ModifyQuery(from, to),
                                            rate_(rate) {}
  ...
  class Factory : public QueryFactory {
   public:
    [[nodiscard]] std::unique_ptr<Query> Construct(std::string_view config) const override {
      auto parts = Split(config, ' ');
      double rate = std::stod(std::string(parts[2])) / 100;
      return std::make_unique<PayTax>(Date(parts[0]), Date(parts[1]), rate);
    }
  };
...
class Spend : public ModifyQuery {
 public:
  Spend(Date from, Date to, double amount) : ModifyQuery(from, to),
                                             amount_(amount) {}
  ...
  void Process(BudgetManager &budget) const override {
    ...
    double day_expenses = amount_ / (idx_to - idx_from + 1);

    for (int i = idx_from; i <= idx_to; ++i) {
      budget.GetDayState(i).spent += day_expenses;
    }
  }
